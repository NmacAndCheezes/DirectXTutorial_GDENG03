...
#pragma region Create Device
	...
	for (UINT driver_type_index = 0; driver_type_index < num_driver_types; ++driver_type_index) {
		...
		if (SUCCEEDED(res)) break; //Succeded to created device
	}

	if (FAILED(res)) throw std::exception("[RenderSystem] Device was not created"); //Failed to create device
#pragma endregion

#pragma region Create DeviceContext
...
bool RenderSystem::compileVertexShader(const wchar_t* file_name, const char* entry_point_name,
